...
pdf_parser_token_t* _pdf_parser_next_one_token(const unsigned char* start, const unsigned char* end)
{
    ...
    do
    {
        ...
        fixed_token* to_compare = fixed_token_map[len].tokens;
        ...
        if (isfind)
        {
            tk = pdf_parser_token_init(start, to_compare[mid].type, len);
            start += len;
            return tk;
        }
        else
